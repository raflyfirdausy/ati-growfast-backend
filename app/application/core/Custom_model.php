...
class Custom_model extends MY_Model
{
        ...
        if ($this->soft_deletes === TRUE) {
            if (isset($to_update) && count($to_update) > 0) {
                foreach ($to_update as &$row) {
                    ...
                    //$row = $this->trigger('before_soft_delete',$row);
                    // $row[$this->_deleted_at_field] = $this->_the_timestamp();    //coment by RAFLY
                    // die(json_encode($to_update));
                    $data[$this->_deleted_at_field] = $this->_the_timestamp(); //? BY RAFLY
                    $this->update($data, $row); //?RFL :  karena timestamp kemungkinan besar affected row = 1
                    $affected_rows++; //? BY RAFLY
                }
    ...
    {
        $data['created_by'] = !empty($_SESSION[SESSION]["m_admin"]) ? $_SESSION[SESSION]["m_admin"]["id"] : null;
        return $data;
    }
