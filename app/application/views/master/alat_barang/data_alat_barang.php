    ...
    foreach ($FIELD_FORM as $form) : $isHideFromTable = isset($form["hideFromTable"]) ? $form["hideFromTable"] : FALSE; ?>
        <?php if ($index == 0) : ?>
            RFL_COLUMNS.push({
                ...
                render: function(data, type, row, meta) {
                    return /* html */ `
                        <a class="btn default btn-outline image-popup-vertical-fit el-link" target="_blank" href="${data ? " <?= base_url(LOKASI_ALAT_BARANG_GAMBAR) ?>" + data : "<?= base_url(GAMBAR_NOT_FOUND) ?>" }">
                            <img src="${data ? " <?= base_url(LOKASI_ALAT_BARANG_GAMBAR) ?>" + data : "<?= base_url(GAMBAR_NOT_FOUND) ?>" }" alt="${row.nama}" class="" width="50" height="50" />
                        </a>
                        `
                }
            })
        <?php elseif ($index == 1) : ?>
            RFL_COLUMNS.push({
                ...
                className: "text-center align-middle",
                render: function(data, type, row, meta) {
                    let btnPemakaian = /*html */ `<a class="btn btn-dark disabled" disabled href="<?= base_url(LOKASI_ALAT_BARANG_PDF) ?>${row.pdf_pemakaian}" class="dropdown-item"><i class="fas fa-file-pdf"></i> Cara Pemakaian</a>`
                    if (row.pdf_pemakaian != undefined && row.pdf_pemakaian != "") {
                        btnPemakaian = /*html */ `<a class="btn btn-success" target="_blank" href="<?= base_url(LOKASI_ALAT_BARANG_PDF) ?>${row.pdf_pemakaian}" class="dropdown-item"><i class="fas fa-file-pdf"></i> Cara Pemakaian</a>`
                    }
                    return btnPemakaian
                }
            })
